...
router.get('/', (req, res) => {
  db.all(`SELECT * FROM apartments WHERE category = 'Sales' AND available = 1`, (err, apartments) => {
    if (err) {
      console.error(err);
      return res.render('category', { apartments: [], category: 'Sales', title: 'Apartments for Sale' });
    }
    ...
    apartments.forEach((apt, index) => {
      getInventoryCount(apt.title, (count) => {
        ...
        if (processed === apartments.length) {
          ...
          res.render('category', { apartments: filtered, category: 'Sales', title: 'Apartments for Sale' });
        }
      });
    });
