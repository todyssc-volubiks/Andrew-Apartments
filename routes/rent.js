...
router.get('/', (req, res) => {
  db.all(`SELECT * FROM apartments WHERE category = 'Rent' AND available = 1`, (err, apartments) => {
    if (err) {
      console.error(err);
      return res.render('category', { apartments: [], category: 'Rent', title: 'Rental Apartments' });
    }
    ...
    apartments.forEach((apt, index) => {
      getInventoryCount(apt.title, (count) => {
        ...
        if (processed === apartments.length) {
          ...
          res.render('category', { apartments: filtered, category: 'Rent', title: 'Rental Apartments' });
        }
      });
    });
